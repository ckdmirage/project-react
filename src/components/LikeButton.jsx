...
const LikeButton = ({ artworkId, authorId }) => {
    ...
    useEffect(() => {
        ...
        // 查點讚數
        axios.get(`http://localhost:8081/like/count/${artworkId}`)
        ...
        // 查是否已點讚
        if (token) {
            axios.get(`http://localhost:8081/like/hasLiked/${artworkId}`, {
    ...
    const handleLikeClick = async () => {
        ...
        try {
            if (hasLiked) {
                await axios.delete(`http://localhost:8081/like/${artworkId}`, {
                    headers: { Authorization: `Bearer ${token}` },
                    withCredentials: true,
                });
                setHasLiked(false);
                ...
                await axios.post(`http://localhost:8081/like/${artworkId}`, {}, {
                    headers: { Authorization: `Bearer ${token}` },
                    withCredentials: true,
                });
                setHasLiked(true);
